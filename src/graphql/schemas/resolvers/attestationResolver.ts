...
class AttestationResolver {
  ...
  async attestations(@Args() args: GetAttestationArgs) {
    try {
      ...
      if (error) {
        console.warn(
          `[AttestationResolver] Errors found while fetching attestations: `,
          error,
        );
      ...
      throw new Error(
        `[AttestationResolver] Error fetching attestations: ${error.message}`,
      );
    }
  }
  ...
  async hypercerts(@Root() attestation: Attestation) {
    ...
    try {
      const res = await this.supabaseService.getHypercerts({
        where: {
          contract: {
            chain_id: { eq: pointer.chain_id },
            contract_address: { eq: pointer.contract_address },
          },
      ...
      if (error) {
        console.warn(
          `[AttestationResolver] Error fetching hypercerts: `,
          error,
        );
        return null;
      }
      ...
      const error = e as Error;
      throw new Error(
        `[AttestationResolver] Error fetching hypercerts: ${error.message}`,
      );
    }
  }
}

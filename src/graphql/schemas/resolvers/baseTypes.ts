import { type ClassType, Field, Int, Resolver, ObjectType, Query } from "type-graphql";
...
export function DataResponse<TItem extends object>(TItemClass: ClassType<TItem>) {
  @ObjectType()
...
export function createBaseResolver<T extends ClassType>(entityFieldName: string, objectTypeCls: T, targetDb: "caching" | "data") {
  ...
  class BaseResolver {
    ...
    getMetadata(args: GetMetadataArgs, single: boolean = false) {
      ...
      try {
        if (single) {
          return this.supabaseCachingService.getMetadata(args).executeTakeFirst();
        }
    ...
    getContracts(args: GetContractsArgs, single: boolean = false) {
      ...
      try {
        if (single) {
          return this.supabaseCachingService.getContracts(args).executeTakeFirst();
        }
    ...
    getHypercerts(args: GetHypercertsArgs, single: boolean = false) {
      ...
      try {
        if (single) {
          return this.supabaseCachingService.getHypercerts(args).executeTakeFirst();
        }
    ...
    getFractions(args: GetFractionsArgs, single: boolean = false) {
      ...
      try {
        if (single) {
          return this.supabaseCachingService.getFractions(args).executeTakeFirst();
        }
    ...
    getAllowlistRecords(args: GetAllowlistRecordsArgs, single: boolean = false) {
      console.debug(
      ...
      try {
        if (single) {
          return this.supabaseCachingService.getAllowlistRecords(args).executeTakeFirst();
        }
    ...
    getAttestationSchemas(args: GetAttestationSchemasArgs, single: boolean = false) {
      console.debug(
      ...
      try {
        if (single) {
          return this.supabaseCachingService.getAttestationSchemas(args).executeTakeFirst();
        }

        return this.supabaseCachingService.getAttestationSchemas(args).execute();
      } catch (e) {
        const error = e as Error;
        throw new Error(
    ...
    async getAttestations(args: GetAttestationsArgs, single: boolean = false) {
      ...
      try {
        if (single) {
          const res = await this.supabaseCachingService.getAttestations(args).executeTakeFirst();
          return res ? this.parseAttestation(res) : null;
        }

        const res = await this.supabaseCachingService.getAttestations(args).execute();
    ...
    getSales(args: GetSalesArgs, single: boolean = false) {
      console.debug(
        `[${entityFieldName}Resolver::getSales] Fetching sales`
      );
